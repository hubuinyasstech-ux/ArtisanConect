...
export function AdminUsersTable({ initialUsers, currentAdminId }: AdminUsersTableProps) {
  ...
  const [selectedProfileUser, setSelectedProfileUser] = useState<AdminUserItem | null>(null);
  ...
  return (
    <div className="space-y-6">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex items-center gap-1.5 justify-end">
                          <button
                            ...
                            className="px-2.5 py-1 rounded-xl text-[11px] font-bold text-slate-700 bg-white border border-slate-200 hover:text-[#ea580c] hover:bg-slate-50 transition-colors flex items-center gap-1 shadow-2xs"
                          ...
                          {u.artisan_profile_id && (
                            <a
                              href={`${process.env.NEXT_PUBLIC_MARKETPLACE_URL || "http://localhost:3000"}/artisans/${u.artisan_profile_id}`}
                              target="_blank"
      ...
      {/* Admin User & Artisan Profile Inspection Modal */}
